package br.com.hefesto.resources.impl.tickets.maintenance.rules;

import br.com.hefesto.domain.impl.Interaction;
import br.com.hefesto.domain.impl.Ticket;
import br.com.wsbasestructure.dto.CustomEmail;
import br.com.wsbasestructure.dto.FlowContainer;
import br.com.wsbasestructure.dto.impl.HefestoAuthenticator;
import br.com.wsbasestructure.dto.interfaces.IHolder;
import br.com.wsbasestructure.rules.interfaces.ICommand;
import java.io.UnsupportedEncodingException;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import javax.mail.MessagingException;
import org.hibernate.Session;

/**
 *
 * @author andrew
 */
public class EmailfyNewTicketToOwnerWhenFinishCommand implements ICommand {

    @Override
    public IHolder exe(IHolder holder, FlowContainer flowContainer) {
        Ticket t = (Ticket) holder.getEntities().get(0);

        if (t.getStatus().equals("Fechado")) {
            /*ANALYSING THE VARIANCES OF THE TICKET TO REPLICATE IN THE EMAIL*/
            SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/YYYY 'às' HH:mm");

            StringBuilder emailBuilder = new StringBuilder();

            String responsible = "";
            if (t.getResponsible() == null) {
                responsible = "Nenhum";
            } else {
                responsible = t.getResponsible().getFullName();
            }

            emailBuilder.append("<!DOCTYPE html><html lang=\"pt-BR\"><head> <meta charset=\"utf-8\"> <title>Resolução de ticket - Hefesto - Sistema de gerenciamento de tickets e solicitações de TI.</title><style> th { border-bottom: 2px solid rgba(0, 0, 0, 0.5); padding: 5px; } td { padding: 15px; } </style></head><body> <div style=\"padding: 0px; margin: 0;\"><img style=\"margin: 0 auto; display: table;\" src=\"data:image/png;base64,\" alt=\"\" /> </div> <div style=\"text-align: center;\"> <h3>Resolução de Ticket</h3> <p> <strong>")
                    .append(t.getOwner().getFullName())
                    .append("</strong>, seu ticket <strong>\"")
                    .append(t.getTitle())
                    .append("\"</strong> de ID <strong>")
                    .append(t.getId())
                    .append("</strong>, foi resolvido no dia ")
                    .append(sdf.format(t.getDateReg()))
                    .append("<p>O responsável pelo seu ticket é o técnico ")
                    .append(responsible)
                    .append(". No entanto, toda a equipe pode ter contribuido para a resolução.</p>")
                    .append("<p>Veja a lista de contribuintes: </p>")
                    .append("<table style=\"margin: 0 auto;\"> <thead> <tr style=\" text-align:left;\"> <th>Data</th> <th>Técnico</th> <th>Ação</th> <th>Alteração Estrutural</th> <th>Atualização</th> </tr> </thead> <tbody>");
            /*LET'S RETRIEVE THE TICKET AGAIN TO ACCESS THE INTERACTIONS*/
            Session s = flowContainer.getSession();
            t = (Ticket) s.get(Ticket.class, t.getId());
            /*PASSING ALL INTERACTIONS IN THE FOR TO CREATE TABLE ROWS*/
            for (Object interaction : t.getInteractions()) {
                Interaction inte = (Interaction) interaction;
                emailBuilder.append("<tr>");
                emailBuilder.append("<td>").append(sdf.format(inte.getDateReg())).append("</td>");
                emailBuilder.append("<td>").append(inte.getUser().getFullName()).append("</td>");
                emailBuilder.append("<td>").append(inte.getType()).append("</td>");
                emailBuilder.append("<td>").append(inte.getTicketChanges()).append("</td>");
                emailBuilder.append("<td>").append(inte.getStringUpdate()).append("</td>");
                emailBuilder.append("</tr>");
            }
            emailBuilder.append("</tbody></table>");
            emailBuilder.append("<br/> <br/> <br/> <br/> <br/> <p><small>Este email foi enviado automaticamente pelo sistema. Não responda este email, pois não receberá feedback.</small></p> </div></body></html>");

            CustomEmail ce = new CustomEmail("smtp", "smtp.office365.com", "587", new HefestoAuthenticator(), "hefesto.noreply@etec.sp.gov.br");
            ce.setBody(emailBuilder.toString());
            ce.setFrom("hefesto.noreply@etec.sp.gov.br");
            ce.setFromName("Hefesto - Sistema de gerenciamento tickets e solicitações de TI");
            ce.setRecipients(new ArrayList());
            ce.getRecipients().add(t.getOwner().getEmail());
            ce.setSender(ce.getFrom());
            ce.setSenderName(ce.getFromName());
            ce.setSubject(new StringBuilder("Resolução do Ticket ").append(t.getId()).append("- Hefesto - Sistema de gerenciamento de tickets e solicitações de TI").toString());

            Runnable runnable = () -> {
                try {
                    ce.send();
                } catch (MessagingException | UnsupportedEncodingException ex) {
                    ex.printStackTrace();
                }
            };

            new Thread(runnable).start();
        }
        return holder;
    }

}
