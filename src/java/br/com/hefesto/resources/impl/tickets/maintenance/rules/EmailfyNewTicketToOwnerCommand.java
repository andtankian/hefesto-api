package br.com.hefesto.resources.impl.tickets.maintenance.rules;

import br.com.hefesto.domain.impl.Ticket;
import br.com.wsbasestructure.dto.CustomEmail;
import br.com.wsbasestructure.dto.FlowContainer;
import br.com.wsbasestructure.dto.impl.HefestoAuthenticator;
import br.com.wsbasestructure.dto.interfaces.IHolder;
import br.com.wsbasestructure.rules.interfaces.ICommand;
import java.io.UnsupportedEncodingException;
import java.text.SimpleDateFormat;
import java.util.ArrayList;
import javax.mail.MessagingException;

/**
 *
 * @author andrew
 */
public class EmailfyNewTicketToOwnerCommand implements ICommand {

    @Override
    public IHolder exe(IHolder holder, FlowContainer flowContainer) {
        Ticket t = (Ticket) holder.getEntities().get(0);
        /*ANALYSING THE VARIANCES OF THE TICKET TO REPLICATE IN THE EMAIL*/
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/YYYY 'às' HH:mm");
        String equipment = t.getEquipment() == null ? "Não se aplica." : t.getEquipment().getName();
        String currentColorPriority;
        if (t.getPriority().equals("P5")) {
            currentColorPriority = "#009688";
        } else if (t.getPriority().equals("P4")) {
            currentColorPriority = "#2196F3";
        } else if (t.getPriority().equals("P3")) {
            currentColorPriority = "#FFC107";
        } else if (t.getPriority().equals("P2")) {
            currentColorPriority = "#FF9800";
        } else {
            currentColorPriority = "#FF9800";
        }

        StringBuilder emailBuilder = new StringBuilder();

        emailBuilder.append("<!DOCTYPE html><html lang=\"pt-BR\"><head>  <meta charset=\"utf-8\">  <title>Novo Ticket - Hefesto - Sistema de gerenciamento de tickets e solicitações de TI.</title></head><body>  <div style=\"padding: 0px; margin: 0;\"><img style=\"margin: 0 auto; display: table;\" src=\"data:image/png;base64,\"      alt=\"\" />  </div>  <div style=\"text-align: center;\">    <h3>Abertura de novo TICKET</h3>    <p>      <strong>")
                .append(t.getOwner().getFullName())
                .append("</strong>, você abriu um novo ticket no dia ")
                .append(sdf.format(t.getDateReg()))
                .append(" com as seguintes informações:    </p>    <p><strong>Título: </strong><span>")
                .append(t.getTitle())
                .append("</span></p>    <p><strong>Descrição: </strong><span>")
                .append(t.getDescription())
                .append("</span></p>    <p><strong>Equipamento: </strong><span>")
                .append(equipment)
                .append("</span></p>    <p>O <span style=\"color:#607D8B; text-transform: uppercase;\">Hefesto</span> automaticamente classificou seu Ticket como prioridade ")
                .append("<span style=\"border-radius: 2px; background-color: ")
                .append(currentColorPriority).append("; padding: 2px 10px; color: white;\">")
                .append(t.getPriority()).append("</span>").append("<br/><br/> Sendo que as prioridades são: </p>    <table style=\"margin: 0 auto;\">      <thead>        <tr>          <th>Classificação</th>          <th>Valor</th>        </tr>      </thead>      <tbody>        <tr>          <td><span style=\"border-radius: 2px; background-color: #009688; padding: 2px 10px; color: white;\">P5</span></td>          <td>Normal</td>        </tr>        <tr>          <td><span style=\"border-radius: 2px; background-color: #2196F3; padding: 2px 10px; color: white;\">P4</span></td>          <td>Média</td>        </tr>        <tr>          <td><span style=\"border-radius: 2px; background-color: #FFC107; padding: 2px 10px; color: white;\">P3</span></td>          <td>Relevante</td>        </tr>        <tr>          <td><span style=\"border-radius: 2px; background-color: #FF9800; padding: 2px 10px; color: white;\">P2</span></td>          <td>Alto</td>        </tr>        <tr>          <td><span style=\"border-radius: 2px; background-color: #F44336; padding: 2px 10px; color: white;\">P1</span></td>          <td>Muito alto</td>        </tr>      </tbody>    </table>    <p>Seu Ticket será analisado pela equipe responsável e você será notificado por email a cada atualização.</p>    <p>Você também pode acompanhar o status do seu Ticket colando o seguinte ID na busca de Tickets: <strong>")
                .append(t.getId()).append("</strong></p>    <br/>    <br/>    <br/>    <br/>    <br/>    <p><small>Este email foi enviado automaticamente pelo sistema. Não responda este email, pois não receberá feedback.</small></p>  </div></body></html>");
        
        CustomEmail ce = new CustomEmail("smtp", "smtp.office365.com", "587", new HefestoAuthenticator(), "hefesto.noreply@etec.sp.gov.br");
        ce.setBody(emailBuilder.toString());
        ce.setFrom("hefesto.noreply@etec.sp.gov.br");
        ce.setFromName("Hefesto - Sistema de gerenciamento tickets e solicitações de TI");
        ce.setRecipients(new ArrayList());
        ce.getRecipients().add(t.getOwner().getEmail());
        ce.setSender(ce.getFrom());
        ce.setSenderName(ce.getFromName());
        ce.setSubject("Abertura de novo Ticket - Hefesto - Sistema de gerenciamento de tickets e solicitações de TI");
        
        Runnable runnable = () -> {
            try {
                ce.send();
            } catch (MessagingException | UnsupportedEncodingException ex) {
                ex.printStackTrace();
            }
        };
        
        new Thread(runnable).start();
        return holder;
    }

}
